// <copyright file="Proxy.ashx.cs" company="LeeftSamen B.V.">
// Copyright © 2015-2016 LeeftSamen B.V. All rights reserved.
// </copyright>

using System.Threading;

namespace LeeftSamen.Portal.Web.ResourceProxy
{
    using System;
    ...
    public class Proxy : IHttpHandler
    {
        class RateMeter
        {
            ...
            {
                this._rate = (double)rate_limit / rate_limit_period / 60;
                this._countCap = rate_limit;
            }

            //called when rate-limited endpoint is invoked
            public bool click()
            {
                TimeSpan ts = DateTime.Now - this._lastUpdate;
                this._lastUpdate = DateTime.Now;
                //assuming uniform distribution of requests over time,
                //reducing the counter according to # of seconds passed
                //since last invocation
                this._count = Math.Max(0, this._count - ts.TotalSeconds * this._rate);
            ...
            {
                TimeSpan ts = DateTime.Now - this._lastUpdate;
                return this._count - ts.TotalSeconds * this._rate <= 0;
            }
        }
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!passThrough && !hasClientToken)
            {
                ...
                if (!tokenIsInApplicationScope)
                {
                    ...
                    {
                        token = this.getNewTokenIfCredentialsAreSpecified(serverUrl, uri);
                    }
                }
            ...
            try
            {
                serverResponse = this.forwardToServer(context, this.addTokenToUri(uri, token, tokenParamName), postBody);
            }
        ...
        */
        ...
        private System.Net.WebResponse forwardToServer(HttpContext context, string uri, byte[] postBody)
        {
            return
                postBody.Length > 0 ?
                this.doHTTPRequest(uri, postBody, "POST", context.Request.Headers["referer"], context.Request.ContentType) :
                this.doHTTPRequest(uri, context.Request.HttpMethod);
        }
        ...
        private System.Net.WebResponse doHTTPRequest(string uri, string method)
        {
            ...
            return this.doHTTPRequest(uri, bytes, method, PROXY_REFERER, contentType);
        }
        ...
        private string getNewTokenIfCredentialsAreSpecified(ServerUrl su, string reqUrl)
        {
            ...
            if (isUserLogin || isAppLogin)
            {
                ...
                if (isAppLogin)
                {
                    ...
                    string tokenResponse = this.webResponseToString(this.doHTTPRequest(uri, "POST"));
                ...
                else
                {
                    ...
                    {
                        string tokenResponse = this.webResponseToString(this.doHTTPRequest(reqUrl, "POST"));
                        token = this.extractToken(tokenResponse, "token");
                        return token;
                    }
                    ...
                    if (infoUrl != "")
                    {
                        ...
                        string infoResponse = this.webResponseToString(this.doHTTPRequest(infoUrl, "GET"));
                        String tokenServiceUri = this.getJsonValue(infoResponse, "tokenServicesUrl");
                        if (string.IsNullOrEmpty(tokenServiceUri))
                        {
                            tokenServiceUri = this.getJsonValue(infoResponse, "tokenServiceUrl");
                        }
        ...
        private string exchangePortalTokenForServerToken(string portalToken, ServerUrl su)
        {
            ...
            string tokenResponse = this.webResponseToString(this.doHTTPRequest(uri, "GET"));
    ...
    public class ServerUrl
    {
        ...
        {
            get { return this.matchAll; }
            set { this.matchAll = value; }
        }

        [XmlAttribute("oauth2Endpoint")]
        public string OAuth2Endpoint
        {
            get { return this.oauth2Endpoint; }
            set { this.oauth2Endpoint = value; }
        }
        ...
        {
            get { return this.tokenParamName; }
            set { this.tokenParamName = value; }
        }

        [XmlAttribute("rateLimit")]
        public int RateLimit
        {
            get { return string.IsNullOrEmpty(this.rateLimit) ? -1 : int.Parse(this.rateLimit); }
            set { this.rateLimit = value.ToString(); }
        }

        [XmlAttribute("rateLimitPeriod")]
        public int RateLimitPeriod
        {
            get { return string.IsNullOrEmpty(this.rateLimitPeriod) ? 60 : int.Parse(this.rateLimitPeriod); }
            set { this.rateLimitPeriod = value.ToString(); }
        }
    }
}